...
class Setting:
    ...
    MRI_DATA_DIR = BASE_DIR / "../../../data/derivatives/"
...
def extract_choice_data(subject_id, run):
    ...
    df = pd.read_csv(data_path / file[0])
    return df


def create_events(choice_data, lr):
    values = np.ones(3) / 2
    ...
    events = pd.DataFrame({"onset": onsets, "duration": 0.0, "trial_type": "outcome_rpe", "modulation": pes})
    return events
...
def main():
    ...
    for subject_id in range(1, 38):
        ...
        for run in range(1, 5):
            ...
            events = create_events(choice_data, lr)
            save_path = Setting.INTERIM_RESULTS_DIR / f"sub-{subject_id:03d}" 
