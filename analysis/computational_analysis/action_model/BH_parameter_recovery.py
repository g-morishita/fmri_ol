import pandas as pd
import numpy as np
from analysis.computational_analysis.simulator import SocialActionModel, AsocialRewardModel
...
STAN_MODEL_PATH = Path("analysis/computational_analysis/stan/action.stan")
...
def simulate_choice_data(n_subjects, n_blocks, n_trials,
                         ...
                         SocialActionModel, AsocialRewardModel,
                         rng=None):
    """
    ...
    for i in range(n_subjects):
        for b in range(n_blocks):
            ...
            model = SocialActionModel(action_lr=lr_i, beta=beta_i)
...
def main(seed):
    ...
    # Fit the Stan model
    results_path = Path(f"results/BH_action_parameter_recovery_model_summary_seed={seed}.csv")
    ...
    df.to_csv(f"results/BH_action_parameter_recovery_seed={seed}.csv", index=False)
