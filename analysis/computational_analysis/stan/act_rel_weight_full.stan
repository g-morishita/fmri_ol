...
parameters {
  ...
  // slope & bias hyperparams (unbounded; will be mapped to (0,1) via Phi later)
  vector[2] mu_latent_slope_weight_for_A;
  vector<lower=0>[2] sigma_latent_slope_weight_for_A;

  vector[2] mu_latent_bias_weight_for_A;
  vector<lower=0>[2] sigma_latent_bias_weight_for_A;
  ...
  matrix[n_subjects, 2] latent_slope_weight_for_A;
  matrix[n_subjects, 2] latent_bias_weight_for_A;
...
transformed parameters {
  ...
  // NOTE: slope & bias should be UNCONSTRAINED (reals)
  matrix[n_subjects, 2] slope_weight_for_A;
  matrix[n_subjects, 2] bias_weight_for_A;
  ...
  for (i in 1:n_subjects) {
    for (c in 1:2) {
      ...
      // hierarchical slope/bias (unbounded)
      slope_weight_for_A[i, c] = mu_latent_slope_weight_for_A[c]
                               + sigma_latent_slope_weight_for_A[c] * latent_slope_weight_for_A[i, c];
      bias_weight_for_A[i, c]  = mu_latent_bias_weight_for_A[c]
                               + sigma_latent_bias_weight_for_A[c]  * latent_bias_weight_for_A[i, c];
...
model {
  ...
  sigma_latent_lr_action  ~ normal(0, 1);

  mu_latent_slope_weight_for_A      ~ normal(0, 1);
  sigma_latent_slope_weight_for_A   ~ normal(0, 1);

  mu_latent_bias_weight_for_A       ~ normal(0, 1);
  sigma_latent_bias_weight_for_A    ~ normal(0, 1);

  mu_latent_beta          ~ normal(0, 1);
  sigma_latent_beta       ~ normal(0, 1);

  to_vector(latent_lr_reward)            ~ normal(0, 1);
  to_vector(latent_lr_action)            ~ normal(0, 1);
  to_vector(latent_slope_weight_for_A)   ~ normal(0, 1);
  to_vector(latent_bias_weight_for_A)    ~ normal(0, 1);
  ...
  // likelihood (inline updates)
  {
    ...
    for (i in 1:n_subjects) {
      for (b in 1:n_blocks) {
        ...
        // subject/cond slope & bias (unbounded reals)
        real slope_i = slope_weight_for_A[i, cond];
        real bias_i  = bias_weight_for_A[i, cond];
        ...
        for (t in 1:n_trials) {
          ...
          if (sc > 0) {
            // reliability r: if oc missing, fall back to 0 (so wA = Phi(bias))
            real r  = (oc > 0) ? tendencies[oc] : 0;
            real wA = Phi(slope_i * r + bias_i);
